...
// biome-ignore lint/style/noNonNullAssertion: Safety of !: if it doesn't exist, it means we're a level 1 ToC. It'll be handled by the if statement below
...
if (level === 1) {
    ...
    item = elements.item
    states.headingsTree.subscribe((value) => {
        tree = value
    })
}
</script>
